// JavaScript Document
/*
1.js的分层（功能）:jquery(tools),组件(ui),应用（app）,mvc(backbone为mvc的库，不用自己写)
...
mv.app.toBanner=function(){
	...
	var oPrevBg=mv.tools.getByClass(oDiv,'prev_bg')[0];
	var oNextBg=mv.tools.getByClass(oDiv,'next_bg')[0];
	
	var oPrev=mv.tools.getByClass(oDiv,'prev')[0];
	var oNext=mv.tools.getByClass(oDiv,'next')[0];
...
mv.app.toRun=function(){
	...
	oPrev.onclick=function(){
		if(iNow==0){
			iNow=aLi.length/2;
			oUl.style.left=-oUl.style.width+'px';
		}
